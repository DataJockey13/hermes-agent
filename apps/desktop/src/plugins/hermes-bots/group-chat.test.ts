...
describe('durable projection', () => {
  ...
  it('normalizes and persists per-room limits with safe legacy defaults', async () => {
    const { chat } = await loadRoom()

    expect(chat.GROUP_CHAT_MAX_MEMBERS).toBe(12)
...
describe('snapshot merge', () => {
  it('syncs per-room limits with the newer identity revision', async () => {
    const { chat } = await loadRoom()
